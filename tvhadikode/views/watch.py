...
class WatchViews(BaseView):
    ...
    @view_config(route_name='service.watch')
    def service_watch(self):
        if self.request.matchdict['service'] != self.request.matchdict['filename']:
            raise exc.HTTPNotFound()
        return self.build_single_m3u(self.service.slug, self.service.unicast_path)

    @view_config(route_name='watch.all')
    def watch_all(self):
        ...
        for service in DBSession.query(Service).order_by(Service.name).all():
            ...
            body += service.unicast_path + "\n"
        return self.make_m3u_response(body, 'tv.hadiko.de.m3u')
